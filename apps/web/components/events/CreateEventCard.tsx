...
export default function CreateEventCard({ className }: { className?: string }) {
  return (
    <Link href="/events/new" className={cn("block group h-full", className)}>
      <Card className="h-full border-dashed ring-0 border-2 border-muted-foreground/20 bg-muted/5 hover:border-primary/50 hover:bg-primary/5 transition-all duration-300 flex flex-col items-center justify-center min-h-[220px] gap-4">
        ...
        <div className="flex flex-col items-center gap-1 text-center px-4">
          ...
          <p className="text-[11px] text-muted-foreground/60 leading-relaxed max-w-[150px]">
            Schedule a new gathering and start selling tickets
          </p>
        </div>
      </Card>
    </Link>
