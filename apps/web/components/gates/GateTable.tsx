...
import type { EventSummary, Gate } from "@tix-seven/types";
import { cn } from "@/lib/utils";

export interface GateWithAssignment extends Gate {
  currentEventId: string | null;
}

interface Props {
  gates: GateWithAssignment[];
  events: EventSummary[];
}
...
export default function GateTable({ gates, events }: Props) {
  ...
  // Only SCHEDULED or ACTIVE events are valid assignment targets
  ...
  return (
    <Table>
      ...
      <TableBody>
        {gates.map((gate) => {
          const assignedEvent = events.find((e) => e.event_id === gate.currentEventId);
            ...
            <TableRow key={gate.gate_id} className="group transition-colors">
              ...
              <TableCell className="py-2 px-3">
                <Select
                  value={gate.status}
                  onValueChange={(v) =>
                    v && handleStatusChange(gate.gate_id, v as "ONLINE" | "OFFLINE")
                  }
                  disabled={isAnyLoading(gate.gate_id)}
                >
                  <SelectTrigger
                    className={cn(
                      "w-fit min-w-25 h-8 text-xs font-medium transition-colors",
                      gate.status === "ONLINE"
              ...
              {/* Assignment select only shows assignable events */}
              <TableCell className="py-2 px-3">
                <Select
                  value={gate.currentEventId ?? "unassigned"}
                  onValueChange={(v) =>
                ...
                >
                  <SelectTrigger className="w-fit min-w-32">
                    <SelectValue>
                      {isLoading(gate.gate_id, "assign")
                        ? "Saving…"
                        : assignedEvent?.name ?? "Unassigned"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
