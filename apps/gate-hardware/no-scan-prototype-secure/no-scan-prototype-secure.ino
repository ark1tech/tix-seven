...
void setup() {
  ...
  Serial.println("System Ready. Send payload via Serial monitor...");
}
...
void loop() {
  if (Serial.available()) {
    ...
    payload.trim();
    if (payload.length() > 0) {
      Serial.println("Accepted Payload: " + payload);
      verifyTicket(payload);
      Serial.println("Ready. Waiting for next payload via Serial...");
    }
  }
}
